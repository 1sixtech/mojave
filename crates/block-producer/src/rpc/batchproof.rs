...
impl SendBatchProofRequest {
    fn get_proof_response(rpc_req_params: &Option<Vec<Value>>) -> Result<Self> {
        ...
        let signed_proof = serde_json::from_value::<SignedProofResponse>(params[0].clone())
            .map_err(|e| Error::BadParams(format!("Invalid SignedProofResponse: {e}")))?;
    ...
    pub async fn call(request: &RpcRequest, context: RpcApiContext) -> Result<Value> {
        ...
        serde_json::to_value("Proof accepted").map_err(|e| Error::Internal(e.to_string()))
    }
}
